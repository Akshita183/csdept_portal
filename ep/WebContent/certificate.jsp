...
<%String email=session.getAttribute("email").toString(); %>
 <div id="navbarcourse">
		...
		Statement st3=con.createStatement();
			ResultSet rs=st1.executeQuery("SELECT * FROM course WHERE price IS NOT NULL");
			int count=0;
			if(valu!=null)
			{
				if(valu.equals("AZ"))
				{
					rs=st1.executeQuery("select * from course WHERE price IS NOT NULL order by cname");
				}
				if(valu.equals("ZA"))
				{
					rs=st1.executeQuery("select * from course WHERE price IS NOT NULL order by cname DESC");
				}
			}
...
<div class="category">Certificate Course</div>
		...
		{
		%>
		<h1 style="color:white">No Certificate Course Present!</h1>
  <% }
		%>
	</div>
