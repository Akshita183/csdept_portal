...
<div>
    ...
    <div class="containercourse" id="course">
		...
		if(search!=null)
		{
			ResultSet rs=st1.executeQuery("select * from course where cname like '%"+search+"%' or category like '%"+search+"%'");
			if(valu!=null)
			{
				if(valu.equals("AZ"))
				{
					rs=st1.executeQuery("select * from course where cname like '%"+search+"%' or category like '%"+search+"%' order by cname");
				}
				else if(valu.equals("ZA"))
				{
					rs=st1.executeQuery("select * from course where cname like '%"+search+"%' or category like '%"+search+"%' order by cname DESC");
				}
			}
				int sea=0;
				while(rs.next())
				...
				<div class="modal-dialog modal-dialog-centered" role="document">
					<div class="modal-content">
						<div class="modal-header">
							<a href="viewstudymaterial.jsp?cid=<%= rs.getString(1)%>"><span class="A" onclick=" "><i
								class="fa fa-book fa-2x" aria-hidden="true"></i></span></a>
									 <span class="B"><i
								class="fa fa-star fa-2x" aria-hidden="true"></i></span> <span class="C"><i
	    ...
	    %>
			...
			{
				%>
				<h1 class="search">No course found!</h1>
				<%
			}
...
<!--  Custom Scripts --> 
    ...
    $(function() {
        ...
        $("#navbarcourse").load("navbarcourse_s.jsp");
